...
import { getHealthOverview, getStudySummary } from '../../../server/repositories/dashboard-repository'

export const dynamic = 'force-dynamic'

export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ range?: string }> }) {
  const range = parseDashboardRange((await searchParams).range)
  const [study, health] = await Promise.all([
  ...
  return (
    <>
      ...
      <DashboardGrid study={study} health={health} />
    </>
  )
}
